<script lang="ts">
  ...
  const createTask = (id: string, theme: string, description: string, image: string, difficulty: number, answer: string): task => {
    return {
      id: id,
      theme_id: theme,
      answer_type: 0,
  ...
  const submitHandler = () => {
    if (tasksData) {
      localStorage.setItem('task', JSON.stringify([...tasksData, createTask(id, theme, description, image, difficulty, answer)]))
    } else {
      localStorage.setItem('task', JSON.stringify([createTask(id, theme, description, image, difficulty, answer)]))
    }
    taskCreated = true
    theme = ''
    difficulty = 0
